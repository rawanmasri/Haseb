...
public class CreatGroupActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		simpleAdpt = new SimpleAdapter(this, adaptList,
	...
	class addMember extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			String memberID = jsonParser.makeHttpRequest(url_add_member, "GET",
					friendToData, user.getApiKey());
			...
			if (!memberID.equals("failed")) {
				...
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						adaptList.add(createPlanet("setting", friendToData));
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						Toast.makeText(getApplicationContext(),
								"Faild in adding member!", Toast.LENGTH_LONG)
								.show();
					}
				});
	...
	class createGroup extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				Group group = new Group(groupname, groupcurrency,
						user.getId(), friendlist);
